...
# ─── yt-dlp options ───────────────────────────────────────────────────────────
# CRITICAL: 'android_music' client is NOT blocked on cloud/Railway IPs.
# The default 'web' client returns IP-locked URLs that fail on any cloud server.
# android_music also returns CDN URLs the browser can fetch directly (no proxy needed).
_YDL_OPTS = {
...
def stream_url(request, video_id):
    """Resolve stream URL and return as JSON (debug/fallback use)."""
    import time
    with _cache_lock:
...
def stream_proxy(request, video_id):
    """
    Resolve audio stream and serve it.

    Uses android_music yt-dlp client — works on Railway/cloud IPs.
    For normal playback: 302 redirect to CDN (browser fetches directly, zero proxy overhead).
    For Range/seek requests: proxy through Django (browser needs byte-range support).
    """
    import time
    ...
    if not audio_url:
        try:
            ...
            with yt_dlp.YoutubeDL(_YDL_OPTS) as ydl:
                ...
                audio_url = _pick_url(info)
            if not audio_url:
                return HttpResponse('No stream URL found', status=500)
            with _cache_lock:
                _stream_cache[video_id] = (audio_url, time.time() + 270)
        except Exception as e:
            return HttpResponse(f'Stream resolution failed: {e}', status=500)

    # No Range header = initial load → redirect (fast, no server overhead)
    if 'HTTP_RANGE' not in request.META:
        resp = HttpResponseRedirect(audio_url)
        resp['Cache-Control'] = 'no-cache'
        return resp

    # Range header = seeking → must proxy to forward byte-range
    headers = {
    ...
    try:
        upstream = req.get(audio_url, headers=headers, stream=True, timeout=20)
...
def _pick_url(info: dict) -> str:
    """Extract the best audio-only URL from a yt-dlp info dict."""
    if info.get('url'):
        return info['url']
